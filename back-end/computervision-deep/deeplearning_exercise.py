...
def predict_exercise(model, image_path, class_names):
    # 6. Inference Example
    ...
    if os.path.exists(image_path):
        ...
        print(f"Test image '{image_path}' not found. Skipping inference.")
        return None, 0
...
if __name__ == "__main__":
    ...
    if not os.path.exists(model_path):
        ...
        # Assuming class names are fixed for now or need to be loaded/inferred
        # For simplicity in this script, we might re-load dataset to get names or hardcode if known
        # But to be safe let's just re-run training logic if we want to be sure about class names
        # Or better, just hardcode for this check:
        class_names = ['pushup', 'squat'] # This is a simplification for the main block
    
    predict_exercise(model, "test_pushup.jpg", class_names)
